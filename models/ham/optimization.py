...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, use_tpu):
  ...
  # print('vars_to_optimize', vars_to_optimize)
  # grads = tf.gradients(loss, vars_to_optimize)
  
  # if FLAGS.freeze_bert:
  #   vars_to_optimize = [v for v in tvars if not v.name.startswith('bert')]
  #   grads = tf.gradients(loss, vars_to_optimize)
  #   (grads, _) = tf.clip_by_global_norm(grads, clip_norm=1.0)
  #   train_op = optimizer.apply_gradients(zip(grads, vars_to_optimize), global_step=global_step)
  # else:
